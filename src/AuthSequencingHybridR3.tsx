...
// ---------------------------------------------------------------------------
// Shared: Read-only sequence rows (view only — no drag, remove, or add)
// ---------------------------------------------------------------------------

function SequenceRows({ product }: { product: ProductData }) {
  const COL = '32px 1fr 90px 150px 80px';
...
function LocationDetailPanel({ location, products }: LocationDetailPanelProps) {
...
function ProductDetailPanel({ productCode, productName, allLocations }: ProductDetailPanelProps) {
  ...
  return (
    <Box sx={{ border: 1, borderColor: 'divider', borderRadius: 2, overflow: 'hidden', bgcolor: 'background.paper' }}>
      ...
      <SequenceRows product={activeProduct} />
    </Box>
  );
}
...
export default function AuthSequencingHybridR3() {
    ...
    <Box sx={{ p: 3, maxWidth: 1100, mx: 'auto' }}>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="body2" color="text.secondary">
          Select a {viewMode === 'location' ? 'location' : 'product'} to view its authorization sequence.
        </Typography>
      </Box>
